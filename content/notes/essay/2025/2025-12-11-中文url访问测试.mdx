...
> 目的：这篇文章用于验证「静态导出 + 目录解码」方案在中文 URL 场景下是否正常可访问。构建后目录会先被 URL 编码，再由 decode-paths.js 还原为中文目录。
...
* [x] 本地 `npm run build`（会自动执行 `postbuild`）  
* [x] 查看 `out/notes/essay/中文url访问测试/` 是否存在 `index.html`  
* [x] 部署 `out` 到 `/memos`，访问中文 URL，确认不再 404  

## 额外说明

<dd>本例故意保留中文文件名与中文 URL，用于验证解码脚本。</dd>
<dd>可在 frontmatter 添加 `slug: essay/zh-url-test`，那就无需依赖解码脚本。</dd>